...
function buildJobHTML(header, sublines, reorderedBullets) {
  ...
  const bullets = (reorderedBullets && reorderedBullets.length > 0 ? reorderedBullets : extractBullets(sublines));
...
function buildCompetencyTags(keywords) {
  return keywords
    ...
    .slice(0, 8)
    .map(k => `<span class="competency-tag">${k.phrase}</span>`)
    .join('\n      ');
}
...
export async function tailor({ jd, cv, meta, opts = {} } = {}) {
  ...
  const html = fillTemplate(loadTemplate(), {
    ...
    SKILLS: skillsSection ? `<div class="skills-grid"><div class="skill-item">${skillsSection.replace(/\n/g, '<br>')}</div></div>` : '',
  });
